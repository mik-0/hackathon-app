...
const app = new Elysia({ adapter: node() })
	...
	.post("/api/upload", async ({ request, set }) => {
		...
		try {
			...
			const uploadPromise = new Promise<void>((resolve, reject) => {
				// Handle file upload
				bb.on('file', async (fieldname: string, fileStream: NodeJS.ReadableStream, info: { filename: string; encoding: string; mimeType: string }) => {
					try {
						...
						if (!isVideo && !isAudio) {
							...
							uploadError = "Only video and audio files are allowed";
							return;
						}
			...
			const { filename, mimeType, size, filePath: uploadedFilePath } = uploadedFileData;
			const fileType: "video" | "audio" = mimeType.startsWith("video/") ? "video" : "audio";
			...
			const mediaFile = new MediaFile({
				...
				fileType,
				durationSec: formFields.duration ? parseFloat(formFields.duration) : undefined,
			...
			if (mediaFileId) {
				try {
					await MediaFile.findByIdAndUpdate(mediaFileId, { status: "error" });
				} catch (dbError) {
	...
	.get("/api/stream/:id", async ({ params, request, set }) => {
		try {
			...
			set.status = 500;
			return { error: "Streaming failed", details: error instanceof Error ? error.message : "Unknown" };
		}
	})
	.listen(3000, () => {
